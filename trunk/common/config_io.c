...
static void read_buf_init(read_buf_t *rb, FILE *f);
...
void read_buf_init(read_buf_t *rb, FILE *f) {
    rb->f = f;
    rb->next = fgetc(f);
}
 ...
 * Test for # or ;
 */
static inline int test_comment(int c) {
    return c == '#' || c == ';';
}

static inline void char_unescape(strlib_buf_t *sb, int c) {
    switch (c) {
        // C strings, except \nnn and \xhh
        case '\\':
        case '\"':
...
static int nparser_get_next_idt(read_buf_t *rb, const char *name,
    ...
    while (state != ST_IDT_END) {
        ...
        if (eol)
            position_inc_line(pos);
        else
            position_inc_char(pos);
    }
    if (r == 1 || r == 2) {
        strlib_rstrip(sb);
    }
...
static int nparser_get_next_value(read_buf_t *rb, const char *name,
    ...
    enum val_states {
        ...
        ST_VAL_SKIP, // Skip until the end of the line
        ST_VAL_END
    // Stop reading
    } state = ST_VAL_START;
    int c;
    ...
    if (state != ST_VAL_IN_QUOTE && state != ST_VAL_IN_QUOTE_ESC && state
            != ST_VAL_IN_UNQUOTED_ESC) {
        return 0;
    }
    else {
